...
const processing = (image: BoardImage) =>
  [image.cpu_status, image.ai_status].some((s) => s === 'pending' || s === 'running')
function Processing({ image }: { image: BoardImage }) {
  return (
    <div className="processing-states" aria-label="Image processing">
      <span data-status={image.cpu_status} title={image.cpu_error ?? undefined}>
        {image.cpu_status === 'succeeded'
          ? 'Thumbnail ready'
          : image.cpu_status === 'failed'
            ? 'Thumbnail failed'
            : 'Making thumbnail…'}
      </span>
      <span data-status={image.ai_status} title={image.ai_error ?? undefined}>
        {image.ai_status === 'succeeded'
          ? 'Visual features ready'
          : image.ai_status === 'failed'
            ? 'Visual features failed'
...
function App() {
  ...
  useEffect(() => {
    ...
    api<Board[]>('/boards')
      .then((result) => {
        if (!alive) return
        setBoards(result)
        if (initialBoard && !result.some((b) => b.id === initialBoard)) {
          setActive(null)
  ...
  useEffect(() => {
    ...
    api<BoardImage[]>(`/boards/${active}/images?limit=60`, { signal: controller.signal })
      ...
      .catch((e) => {
        if (e.name !== 'AbortError') setImageError(e.message)
      })
      .finally(() => {
  ...
  useEffect(() => {
    if (detail) imageDialog.current?.showModal()
  }, [detail])
  ...
  useEffect(() => {
    ...
    if (detail && processing(detail) && !pending.some((image) => image.id === detail.id))
      pending.push(detail)
  ...
  }, [active, boardLoading, images, detail])
  ...
  async function loadMore() {
    ...
    setLoadingMore(true)
    try {
      const result = await api<BoardImage[]>(
        `/boards/${selected}/images?limit=60&offset=${images.length}`,
      )
      if (currentActive.current === selected) {
        setImages((previous) => [...previous, ...result])
        setMore(result.length === 60)
      }
    } catch (e) {
      if (currentActive.current === selected) setImageError((e as Error).message)
    } finally {
      if (currentActive.current === selected) setLoadingMore(false)
    }
  }
  ...
  function choose(files: FileList | File[]) {
    ...
    if (files.length > 50)
      setError(
        'The first 50 files were selected. Add the rest after this batch finishes.',
      )
    ...
    <div className="app-shell">
      ...
      <header className="archive-header">
        <a
          ...
          aria-label="Museboard home"
        >
          museboard<span>∗</span>
        </a>
        <button
          className={`header-link ${!active ? 'current' : ''}`}
          onClick={() => select(null)}
          disabled={busy}
        >
          <span aria-hidden="true">o.</span> Boards <sup>{boards.length}</sup>
        </button>
        ...
        <button className="header-link" onClick={newBoard} disabled={busy || loading}>
          <span aria-hidden="true">+</span> New board
        </button>
        <div className="archive-edition">
          <span>Personal archive</span>
          <span>
            № {String(boards.reduce((n, b) => n + b.image_count, 0)).padStart(3, '0')}
          </span>
        </div>
      </header>
          ...
          <div className="page-heading">
            <div>
              <div className="eyebrow">
                {active ? 'Selected collection' : 'Image collections'}
              </div>
              <h1>{board?.name ?? 'The board index.'}</h1>
              {active && board?.description && <p>{board.description}</p>}
            </div>
            {active ? (
              <button
                className="primary"
              ...
              <button className="primary" onClick={newBoard} disabled={loading}>
                <Plus size={18} />
                New board
              </button>
            )}
          ...
          {loading ? (
            ...
            <>
              <div className="section-line">
                ...
                <span>Collected, in your own order.</span>
              </div>
              <div className="board-grid">
                {boards.map((b, index) => (
                  <button key={b.id} className="board-card" onClick={() => select(b.id)}>
                    <div className="board-cover">
                      {b.cover_image_id ? (
                        <img
                          src={`/api/media/${b.cover_image_id}`}
                          alt=""
                          loading="lazy"
                        />
                      ) : (
                    ...
                    <div className="board-card-caption">
                      ...
                      <span>
                        {b.image_count} {b.image_count === 1 ? 'image' : 'images'}
                      </span>
                    </div>
                    {b.description && <p>{b.description}</p>}
                  </button>
                ))}
                <button className="create-card" onClick={newBoard}>
                  <span>
                    <Plus size={25} />
                  </span>
                  <h3>Create a board</h3>
                  <p>Begin a new collection.</p>
                </button>
              </div>
              {!boards.length && (
                <div className="first-note">
                  ...
                  <div>
                    ...
                    <p>Create a board to collect images, references, and ideas.</p>
                  </div>
                </div>
              )}
            </>
            ...
            <>
              ...
              {uploads.length > 0 && (
                <section className="upload-panel" aria-label="Upload progress">
                  <div className="upload-panel-heading">
                    <strong aria-live="polite">
                      {busy
                        ? 'Adding your images…'
                        : `${uploads.filter((u) => u.status === 'done').length} uploaded · ${uploads.filter((u) => u.status === 'error').length} failed`}
                    </strong>
                    <button disabled={busy} onClick={clearUploads}>
                      Dismiss
                    </button>
                  </div>
                  {uploads.map((item) => (
                    <div className="upload-row" key={item.id}>
                      ...
                      {item.status === 'done' ? (
                        ...
                        <button disabled={busy} onClick={() => void runUploads([item])}>
                          Retry
                        </button>
                      ) : (
              ...
              <div className="section-line">
                <h2>
                  Images <span>({board?.image_count ?? 0})</span>
                </h2>
      ...
      >
        <form onSubmit={createBoard}>
          <button
            ...
            disabled={creating}
            onClick={() => boardDialog.current?.close()}
          >
            <X size={20} />
          </button>
          <div className="eyebrow">NEW COLLECTION</div>
          <h2>Create a board</h2>
          <p>A title, a few words, a place to collect.</p>
          <label>
            Board name
            <input
              name="name"
      ...
      <dialog ref={imageDialog} className="image-modal" onClose={() => setDetail(null)}>
        {detail && (
          <>
            <button
              className="close-button"
            ...
            <img src={detail.original_url} alt={detail.original_filename} />
            <div>
              <h2>{detail.original_filename}</h2>
              <p>
                {detail.width} × {detail.height} · {sizeLabel(detail.byte_size)}
              </p>
              <Processing image={detail} />
              {detail.cpu_error && detail.cpu_status === 'failed' && (
                <p className="file-error">Thumbnail: {detail.cpu_error}</p>
              )}
              {detail.ai_error && detail.ai_status === 'failed' && (
                <p className="file-error">Visual features: {detail.ai_error}</p>
              )}
              <Discovery
                key={detail.id}
                source={detail}
                open={(image) => {
                  ...
                  setImages((previous) =>
                    previous.map((existing) =>
                      existing.id === image.id ? image : existing,
                    ),
                  )
                }}
              />
              <a href={detail.original_url} target="_blank" rel="noreferrer">
